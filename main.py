...
def find_color1(frame): #purple 

    hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    hsv_lowerbound = np.array([124, 71, 212])  
...
def find_color3(frame): #orange
  
    hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    hsv_lowerbound =  np.array([0, 111, 224])
    hsv_upperbound = np.array([29, 255, 255])
    mask = cv2.inRange(hsv_frame, hsv_lowerbound, hsv_upperbound)
...
def find_color2(frame): #blue

    hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    hsv_lowerbound =  np.array([80, 80, 210])
    hsv_upperbound = np.array([114, 255, 255])
    mask = cv2.inRange(hsv_frame, hsv_lowerbound, hsv_upperbound)
    res = cv2.bitwise_and(frame, frame, mask=mask)
    cnts, hir = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
